...
class RobotiqGripper:
    ...
    def activate_and_calibrate(self) -> None:
        """Activates the gripper and auto-calibrates the minimum and maximum positions based on actual motion."""
        self.activate()
        self.auto_calibrate()
    ...
    def auto_calibrate(self, *, log: bool = True) -> None:
        ...
        if RobotiqGripper.ObjectStatus(status) != RobotiqGripper.ObjectStatus.AT_DEST:
            raise RuntimeError(f"Calibration failed opening to start: {status!s}")

        # try to close as far as possible, and record the number
        (position, status) = self.move_and_wait_for_pos(self.get_closed_position(), 64, 1)
        if RobotiqGripper.ObjectStatus(status) != RobotiqGripper.ObjectStatus.AT_DEST:
            raise RuntimeError(f"Calibration failed because of an object: {status!s}")
